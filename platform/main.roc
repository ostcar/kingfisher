...
update_model_for_host = |boxed_model, event_list|
    ...
    |> Result.map_ok(|model| Box.box(model))
    |> err_to_str()
...
err_to_str : Result a _ -> Result a Str
err_to_str = |r|
    when r is
        Ok(v) -> Ok(v)
        Err(ThisLineIsNecessary) -> Err(Inspect.to_str(ThisLineIsNecessary)) # https://github.com/roc-lang/roc/issues/7289
        Err(err) -> Err(Inspect.to_str(err))
